...
namespace VS_Local_Packages_Cleaner
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("Local Visual Studio installation full path: ");
            ...
            else
            {
                ...
                foreach (var pkg in dirPackages.GetDirectories())
                {
                    ...
                    //if (!pkg.Name.Contains("Microsoft.AspNetCore.TargetingPack"))
                    //    continue; // for debug only, comment the line before release
                    //if (!pkg.Name.Contains("Microsoft.VC"))
                    //    continue; // for debug only, comment the line before release
                    ...
                    if (pkg.Name.ToLower().Contains("zh-cn") && !pkgName.ToLower().Contains("arm64")) pkgName += ".zh-cn";
                ...
                // 2) Save package table to sqlite;
                var toDeleteDirectoryNames = DealDataWithSqlite(dtPackages, newfileTimeline, connString.Replace("[PackagesFileName]", _fileName), strDropTableSql, strCreateTableSql, strUpdateSql, strQuerySql).Result;
             ...
             string connString, string strDropTableSql, string strCreateTableSql, string strUpdateSql, string strQuerySql)
        {
            using (var conn = new SQLiteConnection(connString))
            {
                try
                {
                    ...
                    // 2.2) Create an empty table
                    conn.Execute(strCreateTableSql);

                    // 2.3) Loop and insert, save all packages into sqlite;
                    ...
                    // 2.4) Query & update which package should be marked as "ToDelete";
                    await conn.ExecuteAsync(strUpdateSql.Replace("yyyy-MM-dd HH:mm:ss", newfileTimeline.ToString("yyyy-MM-dd HH:mm:ss")));

                    // 2.5) Query and sync data back, from sqlite file to DataTable
